...
import { UnorderedList } from "@ui/prose";
import MathFormula from "@ui/prose/MathFormula";
import { CircuitEvolutionSimulation } from "./figures";
  ...
  return (
    <Accordion variant="prose" title="Parts of the simulator" className="mb-8">
      ...
      <UnorderedList className="text-base text-slate-800 mt-3 mb-0">
        ...
        <li className="text-base">Each NAND gate has two inputs.</li>
        <li className="text-base">A mutation includes a single change of connection (which includes re-connecting whichever node has changed to follow the above rule). No self-connections are allowed.</li>
        <li className="text-base">There is a maximum of 100 mutations or generations (for now).</li>
      </UnorderedList>
    </Accordion>
  )
}
...
This simulator will be part of the [Systems Biology series](/series/systems-biology), where a more thorough walkthrough will be provided. A future version will also include a more complex model showing how evolution can lead to modularity of circuits when the goals are changed.
