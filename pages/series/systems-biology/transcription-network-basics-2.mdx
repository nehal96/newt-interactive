...
import { MdxLayout, PostArticleSubscribe, MathFormula, ImageSeries, UnorderedList } from "../../../components";
...
We can use this when we think of interactions with transcription factors. The number of molecules of protein <MathFormula tex="Y" /> produced per unit of time is dependant on the concentration of <MathFormula tex="X" /> in its active form, <MathFormula tex="X^*" />.
  ...
  <Link href="/series/systems-biology/transcription-network-basics-1">
    <div className="flex flex-col text-lg font-medium border-b border-b-transparent hover:border-b-slate-300">
      ...
      <div className="inline-flex items-center text-slate-800">
        ...
        <span>Transcription Network Basics: Part One</span>
      </div>
    </div>
  </Link>
</div>
