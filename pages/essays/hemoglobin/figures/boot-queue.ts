...
//
// Every Mol* viewer in the essay (anatomy beats, the binding/lean morphs) mounts
// its own plugin — its own WebGL context. Booting one is expensive — parse a PDB,
// build geometry, compile shaders — and doing several on the same frame (e.g. a
// fast scroll past a run of figures) janks the main thread. So every viewer
// awaits a slot here before `createPluginUI` and releases it once its first frame
// is up, capping how many boot at once.
//
