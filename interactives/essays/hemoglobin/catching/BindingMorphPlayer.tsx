...
// MolScript: every iron atom (the heme Fe — the atom the eye should track).
...
function ControlIcon({ kind }: { kind: PlayIcon }) {
  const paths: Record<PlayIcon, string> = {
    ...
    restart:
      "M12 5V2L7 7l5 5V8a6 6 0 1 1-6 6H4a8 8 0 1 0 8-9z",
  };
  return (
...
/**
 * The "pull" interactive: the baked O₂-binding morph with a play/pause control
 * and a draggable progress slider beneath the 3D viewer. React owns the current
 * frame; an effect pushes it into Mol* (coalesced so overlapping commits never
 * flood the engine). Play advances frames over time; the slider scrubs both ways;
...
export default function BindingMorphPlayer({
  ...
  loopRef.current = (now: number) => {
    ...
    const fc = frameCountRef.current;
    const msPerFrame = PLAY_DURATION_MS / Math.max(1, fc - 1);
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      // The pocket as element-colored ball-and-stick (Fe orange, ring N blue,
      // incoming O₂ red), with the Fe emphasized as a larger orange sphere to
      // track — sized to match the anatomy beats exactly.
      await plugin.builders.structure.representation.addRepresentation(
        structure,
        { type: "ball-and-stick", color: "element-symbol" } as any
      );
      // TODO(iron-size): the high→low-spin shrink (deoxy Fe²⁺ ~0.78 Å → oxy
      // ~0.61 Å, a ~22% drop) is described in the prose but not shown — the iron
      // is a fixed FE_EMPHASIS_SIZE here. Decide whether to animate sizeFactor
      // (~0.25 deoxy → ~0.20 oxy across the morph) so the visual matches the text,
      // keeping it consistent with the anatomy beats either way.
      const iron =
      ...
      if (iron) {
        await plugin.builders.structure.representation.addRepresentation(iron, {
          type: "spacefill",
          color: "uniform",
          colorParams: { value: FE_EMPHASIS_HEX },
          typeParams: { sizeFactor: FE_EMPHASIS_SIZE },
        } as any);
      }
