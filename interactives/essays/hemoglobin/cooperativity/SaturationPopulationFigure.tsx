...
export default function SaturationPopulationFigure({ className }: { className?: string }) {
  ...
  return (
    <ChartFigure
      ...
      height={256}
      className={className}
      title="Oxygen saturation as a population average"
      desc="An S-shaped curve of oxygen saturation versus oxygen pressure, with a movable marker, above a row of hemoglobin molecules whose four oxygen seats fill as pressure rises. The curve height equals the fraction of filled seats across the crowd."
      below={
        <>
          {/* current pressure — left-aligned with a fixed-width number so the
              readout never reflows the slider (the old right-aligned value did) */}
