...
export default function AffinityShiftFigure({ className }: { className?: string }) {
  ...
  return (
    <ChartFigure
      ...
      height={210}
      className={className}
      title="Tuning hemoglobin's oxygen affinity"
      desc="The baseline normal-hemoglobin saturation curve with optional comparison curves for acid, carbon dioxide, 2,3-BPG and fetal hemoglobin. Each shifts the curve left or right; the pressure at which it crosses 50% saturation (P50) marks its affinity."
      below={
    ...
    >
      ...
      {/* P50 reference: a marker on the 50% line for the baseline and each visible
          curve, so the reader can watch P50 slide left and right */}
      <text x={PLOT.X0 + 6} y={py(0.5) - 5} fill={CHART.tick} fontSize={10} textAnchor="start">
        P₅₀
      </text>

      {/* baseline curve (always shown) + the active comparison curves */}
      <SaturationCurve p50={P50} />
      {active.map((f) => (
        <SaturationCurve key={f.key} p50={f.p50} stroke={f.color} width={2} />
      ))}

      {/* P50 dots on the 50% line */}
      <Dot x={px(P50)} y={py(0.5)} r={3.5} fill={CHART.curve} />
      {active.map((f) => (
        <Dot key={f.key} x={px(f.p50)} y={py(0.5)} r={3.5} fill={f.color} />
      ))}
    </ChartFigure>
  );
}
