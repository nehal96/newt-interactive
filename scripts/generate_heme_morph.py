...
Two variants are written: the plain pocket (heme + proximal His + O2) for the
"pull" interactive, and a +distal-His version for the later "lean" close-up.

Output:
  public/structures/heme-oxygenation-morph.pdb            (no distal His)
  public/structures/heme-oxygenation-morph-distal-his.pdb (+ distal His)
"""

import math
import os

# --- which pocket to extract (chain A heme + its histidines) ----------------
HEM_CHAIN, HEM_RESSEQ = "A", 142
HIS_CHAIN, HIS_RESSEQ = "A", 87   # proximal His (F8) — the Fe's 5th ligand, dragged
DIST_CHAIN, DIST_RESSEQ = "A", 58  # distal His (E7) — hovers above, steadies the lean
...
# Tilt of the distal O off the Fe–O axis. The Fe–O–O angle is (180 − this), so 60°
# gives the textbook ~120° bent end-on geometry (the earlier 30° read as ~150°,
# too straight — the "angle correct" fix).
O2_BEND_DEG = 60.0
# How far out the O2 starts. Kept short enough that it docks from *below* the
# distal His rather than flying through it; tune in a real browser.
O2_START_FE = 4.0
...
OUT_DISTAL = os.path.join(
    ROOT, "public", "structures", "heme-oxygenation-morph-distal-his.pdb"
)
...
def read_pocket():
    ...
    with open(SRC) as fh:
        for line in fh:
            ...
            elif res == "HIS" and chain == DIST_CHAIN and resseq == DIST_RESSEQ:
                dist.append(Atom(line))
    return hem, his, dist
...
def main():
    ...
    # Precompute each atom's per-frame endpoints (start -> end). The heme + the
    # proximal His are the shared base; the distal His is appended only for the
    # +distal variant.
    base_moves = []  # (atom, start_xyz, end_xyz)
    for a in hem:
        if a.name == "FE":
            base_moves.append((a, a.xyz, fe_target))
        elif a.name in CORE_NAMES:
            perp_d = dot(sub(a.xyz, centroid), n)
            flat = sub(a.xyz, scale(n, FLATTEN_FRAC * perp_d))
            base_moves.append((a, a.xyz, flat))
        else:
            base_moves.append((a, a.xyz, a.xyz))  # substituents static
    for a in his:
        base_moves.append((a, a.xyz, add(a.xyz, fe_disp)))  # proximal His dragged
    # distal His is static — it only hovers/steadies (omitted from the plain variant).
    distal_moves = [(a, a.xyz, a.xyz) for a in dist_his]

    def write_morph(path, moves, note):
        lines = [
            "REMARK  Synthetic heme-oxygenation morph (deoxy -> oxy), %s." % note,
        ...
        print("Wrote %d frames (%d atoms each) -> %s"
              % (N_FRAMES, len(moves) + 2, os.path.relpath(path, ROOT)))

    write_morph(OUT, base_moves, "no distal His")
    write_morph(OUT_DISTAL, base_moves + distal_moves, "with distal His")
